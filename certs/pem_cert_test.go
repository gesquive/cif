...
-----END CERTIFICATE-----`
	...
	cert := certs[0]
	expectedLabel := "StartCom Certification Authority"
	...
	if cert.Label != expectedLabel {
		t.Errorf("Incorrect name: got %v, expected %v", cert.Label, expectedLabel)
	}
	if cert.Serial != "1" {
...
-----END CERTIFICATE-----`
	...
	for _, cert := range certs {
		...
		if cert.Label != expectedLabel {
			t.Errorf("Incorrect label: got %v, expected %v", cert.Label, expectedLabel)
		}
		if cert.Serial != "1" {
			t.Errorf("Incorrect serial: got %v, expected 1", cert.Serial)
		}
